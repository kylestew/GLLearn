...
static void calculate_flag_vertex(
    ...
    GLfloat
        ...
        tgrad[3] = {-(0.0625f + 0.03125f * sinf((GLfloat)M_PI * time)) * (1.0f - s) * (2.0f * t - 1.0f), 0.75f, 0.0f};
...
struct flag_vertex *init_flag_mesh(struct flag_mesh *out_mesh) {
    struct flag_vertex *vertex_data = (struct flag_vertex *)malloc(FLAG_VERTEX_COUNT * sizeof(struct flag_vertex));
...
#define _FLAGPOLE_T(y) \
    (TEX_FLAGPOLE_LO[1] + (TEX_FLAGPOLE_HI[1] - TEX_FLAGPOLE_LO[1]) * ((y)-FLAGPOLE_TRUCK_TOP) / (FLAGPOLE_SHAFT_BOTTOM - FLAGPOLE_TRUCK_TOP))
...
#undef _FLAGPOLE_T
    ...
    GLsizei
        ...
        vertex_count = flagpole_vertex_count + wall_vertex_count + ground_vertex_count;
    ...
    GLsizei
        ...
        element_count = flagpole_element_count + wall_element_count + ground_element_count;

    struct flag_vertex *vertex_data = (struct flag_vertex *)malloc(vertex_count * sizeof(struct flag_vertex));
